...
function CampaignModal({ agencies, creators, platforms, deliverableTypes, onClose, onSaved }) {
  ...
  async function generateCampaignName(creatorId, agencyId) {
    ...
    // Query existing campaigns today with same agency to determine serial
    const { data: existing } = await supabase
      .from('campaigns')
      .select('campaign_name')
      .eq('agency_id', agencyId)
      .like('campaign_name', `${dateStr}-%`);

    const serial = String((existing?.length || 0) + 1).padStart(2, '0');

    const generated = `${dateStr}-${serial}-${creatorName}-${agencyName}`;
    setForm(f => ({ ...f, campaign_name: generated }));
        ...
        <div className="modal-body">
          ...
          <div className="form-row">
            <div className="form-group">
              ...
              <input
                ...
                onChange={e => { setF('campaign_name', e.target.value); setNameAutoGenerated(false); }}
                placeholder="Select creator + agency to auto-generate"
              />
            </div>
            <div className="form-group">
          ...
          <div className="form-row">
            <div className="form-group">
              ...
              <select className="form-select" value={form.creator_profile_id} onChange={e => {
                const id = e.target.value;
                setF('creator_profile_id', id);
                if (id && form.agency_id) generateCampaignName(id, form.agency_id);
              }}>
            ...
            <div className="form-group">
              ...
              <select className="form-select" value={form.agency_id} onChange={e => {
                const id = e.target.value;
                setF('agency_id', id);
                if (id && form.creator_profile_id) generateCampaignName(form.creator_profile_id, id);
              }}>
