...
export default function OverviewPage({ isAdmin, profileId, creatorName, setActiveView, navigateToCampaign, refreshKey }) {
  ...
  const campaignMonths = [...new Set(viewSummaryRows.map(r => r.invoice_date?.slice(0, 7)).filter(Boolean))].sort().reverse();
  const rewardMonths   = [...new Set(viewRewardRows.map(e => e.period_month?.slice(0, 7)).filter(Boolean))].sort().reverse();
